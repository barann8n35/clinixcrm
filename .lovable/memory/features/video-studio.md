---
name: Video Studio
description: Premium + Premium+ video translation module — subtitles, TTS dub, voice cloning, lip-sync, and client-side subtitle burn-in
type: feature
---
...
**Subtitle burn-in (client-side)**: Edge functions can't run FFmpeg, so subtitle videos are burned in the browser via `@ffmpeg/ffmpeg` wasm. Helper at `src/lib/burnSubtitles.ts` lazy-loads ffmpeg-core from unpkg CDN, uses `subtitles=` filter with libx264 + AAC re-encode. Vite config excludes `@ffmpeg/ffmpeg` and `@ffmpeg/util` from optimizeDeps. Two entry points:
- VideoStudio: Film icon next to SRT download triggers `downloadBurnedVideo` → downloads MP4 to user.
- SendVideoDialog: For subtitle mode, burns once and caches at `clinic-videos/{user_id}/burned/{translation_id}.mp4`, then sends signed URL via WhatsApp message. Subsequent sends reuse the cached file.
...
Patient integration: SendVideoDialog component in PatientPanel — lists user's completed translations and inserts WhatsApp message with video link. Subtitle mode auto-burns + uploads on first send.

Storage path convention: {user_id}/originals/, {user_id}/subtitles/, {user_id}/dubs/, {user_id}/voice-samples/, {user_id}/burned/. Signed URLs valid 1 year.
